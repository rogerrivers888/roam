...
| `ROAM_OVERPASS_URLS` / `ROAM_NOMINATIM_URL` | no | override the OpenStreetMap endpoints (e.g. a self-hosted mirror). **Planet-wide instances only** — a regional extract answers 200, fast, and empty for everywhere else, which no health check can tell from a real result. `overpass.osm.ch` is Switzerland only and cost an afternoon proving it. |
| `ROAM_OVERPASS_TIMEOUT_MS` / `ROAM_STAYS_OPEN_DEADLINE_MS` | no | how long one mirror gets on an interactive path (default 12000), and how long the Stay tab waits for the open map before drawing the list without it (default 10000). |
