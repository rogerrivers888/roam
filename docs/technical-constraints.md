...
**The score is region-relative, and that is a trap as well as a feature.** Readership is measured against the best-read place in the *same region*, which is what makes a single county's list right — Canterbury Cathedral leads Kent rather than being flattened by a log scale into the middle of eleven castles. But it means every county's best thing scores near one, so the moment a search pools several counties the comparison stops meaning anything. The home screen's 60 km radius around Ascot pooled Berkshire with London, and London — a hundred published attractions, all scoring highly — drowned it: the answer to "what shall we do near Ascot" opened with the Tower of London 42.9 km away, and twelve of the first twenty were central London.

The fix is in `publishedNear`, not in the score: ordering damps by distance, gently, so that a place at the far edge of a search keeps 60% of its score. Enough for something exceptional an hour away to hold its own against something merely good round the corner; not enough for it to lead. **Anything that ranks across more than one region has to do something equivalent.** A score comparable within a region is not comparable between them.

**"Listed" is not "finished".** `regions.harvest_state = 'done'` means the county has been listed, and for a long time nothing tracked the other half. Raising Berkshire's `target_count` from 18 to 250 published 232 more attractions in one re-rank; the image pass covered a quarter of them before a deploy restarted the API, and the county still counted as done with 186 places holding no photograph. Nothing would ever have gone back for them. The resume now treats a region with published-but-unillustrated attractions as outstanding, and `attractions.image_state` (migration 039) records that somebody has looked — without which every pass re-examines the hopeless cases for ever and the harvest never converges.
