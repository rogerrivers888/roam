 ...
 */
export function RangeSlider({ min, max, step = 10, low, high, onChange, format = (v) => `£${v}`, bars }: {
  ...
  return (
    <View style={styles.wrap} onLayout={(e) => { widthRef.current = e.nativeEvent.layout.width; setWidth(e.nativeEvent.layout.width); }}>
      {bars?.length ? (
      ...
      {width > 0 ? <View style={[styles.fill, { left: toX(low), width: Math.max(0, toX(high) - toX(low)) }]} pointerEvents="none" /> : null}
      {width > 0 ? (
        <>
          <View {...lowPan.panHandlers} style={[styles.knob, { left: toX(low) - 14 }]} accessibilityRole="adjustable" accessibilityLabel={`From ${format(low)}`} />
          <View {...highPan.panHandlers} style={[styles.knob, { left: toX(high) - 14 }]} accessibilityRole="adjustable" accessibilityLabel={`To ${format(high)}`} />
