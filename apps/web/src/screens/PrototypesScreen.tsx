...
const PROTOTYPES: { file: string; section: Section; title: string; when: string; what: string }[] = [
  { file: 'group-charges.html', section: 'trips', title: 'A cost, from adding it to closing it', when: '4 Sep 2026', what: 'The setup page showed a coach being edited and never added. Three screens to add one — what it is and who pays a share (every head or every party, everyone or only those who want it), how the price works, and when it closes. A varying price is typed the way an organiser thinks: the most anybody pays (£30), the least (£15), the fewest it runs with (12) and how many you expect (24) — from which Roam works out the £360 and the price at every headcount between. The minimum lives at two levels — the weekend\'s own (off by default) and the coach\'s — and the organiser says what missing the coach\'s one means: the coach is off and the weekend carries on, the whole weekend is off, or ask me on the day. Nobody pays while the price can still move; the closing day fixes the headcount, sends a bill that shows its own arithmetic, and is the day it stops being refundable. Plus the morning it closes, the morning it is four short, late joiners up to the seats booked, and what everybody reads when something is called off.' },
  { file: 'group-setup-options.html', section: 'trips', title: 'Setting a group up — the four questions, three ways', when: '4 Sep 2026', what: 'The built Group tab opens on “Still to chase” with nobody to chase, so this re-does the setup: what is mandatory and what is only being asked about, a real scannable QR with the link and a WhatsApp button beside it, how often Roam chases, and — the part with no answer in the app at all — what you are charging for. Charging becomes a list rather than a fee, priced as the same amount for everyone or as a total split between whoever comes (£600 coach: £50 each at 12, £25 at 24), per head or per party, with a minimum number for it to happen and what everybody is told if it is not met. Wizard, one guided page, or set up on the trip itself — and the four states the Group tab has, only two of which are about chasing.' },
  { file: 'group-trips.html', section: 'trips', title: 'Group trips — the organiser\'s six weeks, the participant\'s two minutes', when: '4 Sep 2026', what: 'The direction from the options page drawn end to end, on one trip: twenty-four people to Cardiff, one hotel everybody books themselves, one stadium tour, and £45 each for the coach. Seven light frames for the organiser (a dashed row on a trip he already has, naming it, the checklist proposed from the trip, the link and a code, the empty group, one chasing sheet with the words shown before they go, one person opened) and five dark frames for a participant who has never heard of Roam. Then the same screen at 1000px, what is kept against what is rented, and the six build steps — the first five of which need no payment provider, no partner and no key.' },
  { file: 'group-trips-options.html', section: 'trips', title: 'Group trips — how the group is run, three ways', when: '4 Sep 2026', what: 'The Group Trips v1.0 requirements taken apart: what a group is in Roam\'s model (household, group, participant, checklist item — and what an organiser must never see), three organiser screens (people first, outstanding first, the trip carries it), three ways to join from a link, the £45 and the verification gate that governs it, and the two things a spreadsheet cannot do — a room booked on the wrong night, and three allergies in front of the kitchen for a table of twenty-six. Ends with the eight calls that change what gets built, each with a recommendation.' },
