...
const PROTOTYPES: { file: string; section: Section; title: string; when: string; what: string }[] = [
  { file: 'group-charges.html', section: 'trips', title: 'A cost, from adding it to closing it', when: '4 Sep 2026', what: 'The setup page showed a coach being edited and never added. Three screens to add one — what it is and who pays a share (every head or every party, everyone or only those who want it), how the price works, and when it closes. A varying price is typed the way an organiser thinks: the most anybody pays (£30), the least (£15), the fewest it runs with (12) and how many you expect (24) — from which Roam works out the £360 and the price at every headcount between. Nobody pays while it can still move; the closing day fixes the headcount, sends a bill that shows its own arithmetic, and is the day it stops being refundable. Plus the morning it closes, the morning it is four short, late joiners up to the seats booked, and the six calls to make.' },
