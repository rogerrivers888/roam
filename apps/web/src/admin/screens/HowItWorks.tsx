...
const SECTIONS: Section[] = [
  {
    ...
    decisions: [
      ...
      },
      {
        ...
        rule: 'Pool, kitchen, parking, family room, breakfast, air con and pet-friendly are read off the open map’s tags. A must-have narrows the list only where somebody around there has answered it: if no bed in the pool carries the word at all, it is not applied and the wizard says so in a line under the chips. Where some beds do carry it, they are the answer. Nice-to-haves reorder and never remove, which the screen says under the kicker. The button carries the live count either way.',
        why: 'OpenStreetMap has a tag for a pool and no tag for the absence of one, and the tagging is sparse — in Windsor not one bed carries parking. A strict filter would empty the list everywhere the map is thin, which is most places, and an empty list is not a truer answer than a full one. Saying "nobody has mapped parking around here" is both true and useful; silently returning nothing is neither. LiteAPI’s list endpoint carries no facilities — the per-hotel detail call does, and that is a call per row, which is why it is not made.',
        state: 'partial',
