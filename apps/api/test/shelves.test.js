 ...
 * football stadiums. That's not what I consider adrenaline" (5 Sep 2026). The
 * three things that has to mean are all silent failures if they regress: a
 * stadium is Fun, a circuit is Adrenaline, and nothing lands on four shelves at
 * once. None of them shows up as an error — the home screen just quietly fills
 * a shelf with the wrong places again.
 *
...
const STADIUM = { scope: 'kind', subject: 'Q1154710', subject_label: 'association football venue', weights: { fun: 0.9 } };
const CIRCUIT = { scope: 'kind', subject: 'Q2338524', subject_label: 'motorsport racing track', weights: { adrenaline: 1, fun: 0.7 } };

test('a football ground is a day out, not a thrill', () => {
...
test('an untaught sports venue falls short of Adrenaline rather than onto it', () => {
  // The whole complaint in one assertion: with nothing taught, `active` must
  // not be enough on its own to put a place on the Adrenaline shelf.
...
test('watching a match is not adrenaline in the live pool either', () => {
  assert.deepEqual(shelvesForVenue({ category: 'attraction', experiences: ['sports-game'] }, NO_RULES).shelves, ['fun']);
});
